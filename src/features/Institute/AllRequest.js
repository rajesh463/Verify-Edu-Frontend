...
const AllRequest = () => {
  ...
  const onView = (qualId) => {
    if (qualId) navigate(`/institute/verify-form/${qualId}`);
  };
  return (
    <div className="verify-documents-container">
      <h2 className="verify-documents-title">Verification Requests</h2>
          ...
          <tbody>
            {verifications.map((verification) => (
              <tr key={verification._id}>
                ...
                <td>
                  <button onClick={() => onView(verification.qualificationId)}>
                    View
                  </button>
                </td>
              </tr>
